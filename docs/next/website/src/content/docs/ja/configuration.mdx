...
範囲は 9 より手前で終えることもできます。`1..3` は最初の 3 つの数字だけをバインドします。優先度のショートカットはこの形式を使います:
...
エージェントとスペースには、ユーザーが設定する優先度 `high`、`normal`（デフォルト）、`low` があります。優先度のショートカットは `1` で high、`2` で normal、`3` で low になります。エージェント用のショートカットはフォーカス中のエージェントに、スペース用のショートカットはアクティブなスペース（ナビゲートモードでは選択中のスペース）に適用されます。

サイドバーは優先度を最初に見て並べ替えます。high のエージェントはアテンション順より上に来ます。スペースのブロック（親とその worktree の子）は 1 つのまとまりとして、その中の最も高い優先度で並びます。high のエージェントには `▲`、low のエージェントには `▽` が付きます。各スペース行には番号付きのバッジが付き、high では強調され、low では暗く表示されます。優先度は再起動後も保持され、API の `agent.importance.set` と `workspace.importance.set` からも設定できます。
