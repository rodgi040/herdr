...
A range may stop before nine — `1..3` binds only the first three digits. The priority shortcuts use that form:

```toml
[keys]
agent_importance = "prefix+ctrl+1..3"
workspace_importance = "prefix+ctrl+shift+1..3"
```

The legacy `[keys.indexed]` table is still parsed for compatibility, but new configs should prefer the explicit action fields.

## Agent and space priority

Agents and spaces carry a user-assigned priority: `high`, `normal` (the default), or `low`. Press the priority shortcut with `1` for high, `2` for normal, and `3` for low; the agent shortcut applies to the focused agent, the space shortcut to the active space (or the selected one in navigate mode).

The sidebar sorts by priority first: high agents rise above the attention order, and a space block — a parent plus its worktree children — moves as a unit ranked by its most important member. High agents are marked `▲` and low agents `▽`; each space row carries a numbered badge that is highlighted for high priority and dimmed for low. Priorities survive restarts and can also be set through the API with `agent.importance.set` and `workspace.importance.set`.
